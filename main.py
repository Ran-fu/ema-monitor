...
def send_telegram_message(text):
    try:
        ...
        print(f"🔹 發送前 Token: {token[:5]}... Chat ID: {chat_id}")  # 只顯示前5位保密
        r = requests.post(
    ...
    if today_date != now_date:
        ...
        try:
            ...
            today_top3_up = df[~df['instId'].str.replace("-USDT-SWAP","").isin(main_symbols)]\
                .sort_values('change_pct', ascending=False)['instId'].head(3).str.replace("-USDT-SWAP","").tolist()
            today_top3_down = df[~df['instId'].str.replace("-USDT-SWAP","").isin(main_symbols)]\
                .sort_values('change_pct', ascending=True)['instId'].head(3).str.replace("-USDT-SWAP","").tolist()
...
send_telegram_message(
    "🚀 OKX EMA 吞沒監控已啟動 ✅\n(以收盤K線判斷吞沒)\n"
    + ("今日 漲幅 Top3: " + ", ".join(today_top3_up) if today_top
