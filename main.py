...
state = {
    ...
    "meta": {"uptime_alert_sent": False}
}
last_check_time = None
UPTIME_PING_URL = os.environ.get("UPTIME_PING_URL")  # 選填
...
def get_top_movers():
    ...
    try:
        ...
        if not tickers: return today_list
        ...
        if not sym_col or not change_col: return today_list
...
def check_signals(force=False):
    ...
    try:
        ...
        for sym in symbols:
            ...
            if df30 is not None and len(df30) >= 60: process_signal(sym, df30, "30m")
            # 15 分
            df15 = get_klines(sym, "15min", 200)
            if df15 is not None and len(df15) >= 60: process_signal(sym, df15, "15m")
        ...
        if not force:
            try: send_telegram("⚠️ 系統錯誤：check_signals 失敗")
            except: pass
...
def send_startup_report():
    global today_list
    msgs = []
    for sym in today_list:
        msg_parts = [f"📊 {sym} 開機檢查結果："]
        for tf in ["15m", "30m"]:
            df = get_klines(sym, "15min" if tf=="15m" else "30min", 2)
            if df is None or len(df)<2: continue
            prev = df.iloc[-2]; cur = df.iloc[-1]
            bull_ok = cur["close"] > cur["open"] and prev["close"] < prev["open"] and bullish_pullback_ok(cur)
            bear_ok = cur["close"] < cur["open"] and prev["close"] > prev["open"] and bearish_pullback_ok(cur)
            if bull_ok: msg_parts.append(f"{tf} 🟢看漲")
            elif bear_ok: msg_parts.append(f"{tf} 🔴看跌")
            else: msg_parts.append(f"{tf} —")
        msgs.append("\n".join(msg_parts))
    if msgs: send_telegram("🚀 開機 Top3 EMA 檢查完成\n" + "\n\n".join(msgs))
...
def monitor_health():
    ...
    try:
        ...
        try:
            ...
            if r.status_code!=200: problem=True; reason.append(f"/ping 回傳 {r.status_code}")
    ...
    except Exception as e: print("monitor_health error:", e)
...
if __name__=="__main__":
    ...
    try:
        ...
        send_startup_report()
    except Exception as e: print("initial check error:", e)
    try:
