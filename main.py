# main.py (整合版：15m+30m + Uptime + 開機強制發訊號)
...
# === 處理訊號 (可強制發訊號) ===
def process_signal(sym, df, timeframe, force_send=False):
    ...
    # 多頭
    try:
        bull_ok = cur["ema12"] > cur["ema30"] > cur["ema55"] and bullish_pullback_ok(cur) and is_bull(prev["open"], prev["close"], cur["open"], cur["close"])
        if bull_ok and (force_send or bull_key not in state["signals"]):
            msg = f"🟢{sym} [{timeframe}] {rank}\n看漲吞沒\n收盤: {cur['close']} ({cur['ts']})"
            send_telegram(msg)
            state["signals"][bull_key] = time.time()
            print("Bullish signal sent:", bull_key)
    except Exception as e:
        print("process_signal bull error:", e)

    # 空頭
    try:
        bear_ok = cur["ema12"] < cur["ema30"] < cur["ema55"] and bearish_pullback_ok(cur) and is_bear(prev["open"], prev["close"], cur["open"], cur["close"])
        if bear_ok and (force_send or bear_key not in state["signals"]):
            msg = f"🔴{sym} [{timeframe}] {rank}\n看跌吞沒\n收盤: {cur['close']} ({cur['ts']})"
            send_telegram(msg)
            state["signals"][bear_key] = time.time()
            print("Bearish signal sent:", bear_key)
    except Exception as e:
        print("process_signal bear error:", e)

# === 主檢查 (15m + 30m) ===
def check_signals(force_send=False):
        ...
        for sym in symbols:
            try:
                ...
                if df30 is not None and len(df30) >= 60:
                    process_signal(sym, df30, "30m", force_send)

                df15 = get_klines(sym, "15min", 200)
                if df15 is not None and len(df15) >= 60:
                    process_signal(sym, df15, "15m", force_send)
...
if __name__ == "__main__":
    ...
    try:
        check_signals(force_send=True)  # 開機立即跑一次並強制發訊號
    except Exception as e:
