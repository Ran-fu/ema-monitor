...
def self_ping():
    try:
        url = f"http://127.0.0.1:{os.environ.get('PORT', 8080)}/ping"
        r = requests.get(url, timeout=5)
        if r.status_code == 200:
            print("🏓 自我 ping 成功")
        else:
            print(f"⚠️ 自我 ping 回傳 {r.status_code}")
    except Exception as e:
...
scheduler.add_job(self_ping, 'interval', minutes=5)  # 自我 ping 排程
scheduler.start()
...
send_telegram_message(
    "🚀 OKX EMA 吞沒監控已啟動 ✅\n(以收盤K線判斷吞沒)\n"
    + ("今日 漲幅 Top3: " + ", ".join(today_top3_up) if today_top3_up else "無 Top3")
    + "\n" + ("今日 跌幅 Top3: " + ", ".join(today_top3_down) if today_top3_down else "無 Top3")
)
check_signals()
check_timezone()
