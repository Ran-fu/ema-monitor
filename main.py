...
def update_today_top3():
    global today_top3, today_date, top3_gainers, top3_losers
    ...
    if today_date != now_date:
        ...
        try:
            ...
            df_top = df[~df['instId'].str.replace("-USDT-SWAP","").isin(main_symbols)]
            
            # 漲幅前三
            top3_gainers = df_top.sort_values('change_pct', ascending=False)['instId'].head(3).str.replace("-USDT-SWAP","").tolist()
            # 跌幅前三
            top3_losers = df_top.sort_values('change_pct', ascending=True)['instId'].head(3).str.replace("-USDT-SWAP","").tolist()
            # 結合成 watch_symbols
            today_top3 = list(set(top3_gainers + top3_losers))

            print(f"📈 漲幅前三: {top3_gainers}")
            print(f"📉 跌幅前三: {top3_losers}")
        except Exception as e:
...
def check_signals():
    ...
    main_symbols = ["BTC","ETH","SOL","XRP"]
    watch_symbols = list(set(main_symbols + today_top3))  # 保證 main_symbols 永遠監控
    ...
    for bar in ["15m", "30m"]:
        for symbol in watch_symbols:
            ...
            # 判斷 Top3 類型
            if symbol in top3_gainers:
                prefix_bull = "🔥 Top3 漲幅 "
                prefix_bear = "🔥 Top3 漲幅 "
            elif symbol in top3_losers:
                prefix_bull = "🔻 Top3 跌幅 "
                prefix_bear = "🔻 Top3 跌幅 "
            else:
                prefix_bull = "🟢"
...
@app.route('/')
def home():
    top3_text = f"📈 漲幅前三: {', '.join(top3_gainers)} | 📉 跌幅前三: {', '.join(top3_losers)}" if today_top3 else "尚未更新"
...
load_state()
update_today_top3()
send_telegram_message("🚀 OKX EMA 吞沒監控已啟動 ✅\n(以收盤K線判斷吞沒)\n" + ("今日 Top3 已更新" if today_top3 else "無 Top3"))
check_signals()
check_timezone()
