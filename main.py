...
def update_today_top3():
    ...
    if today_date != now_date:
        ...
        try:
            ...
            # 排除 main_symbols，避免 Top3 覆蓋固定幣種
            main_symbols = ["BTC","ETH","SOL","XRP"]
            today_top3 = df[~df['instId'].str.replace("-USDT-SWAP","").isin(main_symbols)]['instId'].head(3).str.replace("-USDT-SWAP","").tolist()
            print(f"📊 今日 漲幅 Top3: {today_top3}")
        except Exception as e:
