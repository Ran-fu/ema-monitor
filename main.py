...
def update_today_top3():
    ...
    if today_date == today:
        return
    today_date = today
    r = requests.get("https://www.okx.com/api/v5/market/tickers?instType=SWAP", timeout=10).json()
    ...
    today_top3_up = df.sort_values("pct", ascending=False).head(3)["instId"].str.replace("-USDT-SWAP","").tolist()
    today_top3_down = df.sort_values("pct").head(3)["instId"].str.replace("-USDT-SWAP","").tolist()
...
def check_signals():
    ...
    for bar in ["15m", "30m"]:
        for symbol in symbols:
            ...
            if (curr["EMA12"] > curr["EMA30"] > curr["EMA55"] and prev["close"] < prev["open"]
                and curr["close"] > curr["open"] and curr["close"] > prev["open"] and curr["open"] < prev["close"]
                and curr["low"] > curr["EMA55"] and bull_key not in sent_signals):
                send_telegram(f"🟢 {symbol} {tag}\n[{bar}] 看漲吞沒\n收盤 {curr['close']} ({t})")
                sent_signals[bull_key] = datetime.utcnow()
            if (curr["EMA12"] < curr["EMA30"] < curr["EMA55"] and prev["close"] > prev["open"]
                and curr["close"] < curr["open"] and curr["close"] < prev["open"] and curr["open"] > prev["close"]
                and curr["high"] < curr["EMA55"] and bear_key not in sent_signals):
                send_telegram(f"🔴 {symbol} {tag}\n[{bar}] 看跌吞沒\n收盤 {curr['close']} ({t})")
...
def webhook():
    try:
        ...
        if text.strip() == "/myid":
            send_telegram(f"你的 Chat ID 是：{chat_id}", chat_id=chat_id)
    except Exception as e:
