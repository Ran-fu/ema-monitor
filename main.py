...
# === 30m 共振記錄（新增） ===
recent_30m_reversals = {}   # {symbol: {"type": "LOW/HIGH", "time": datetime}}
...
def check_reversal_pattern(df, idx, lookback=20):
    ...
    gravestone = body <= rng * 0.1 and upper >= rng * 0.6 and lower <= body
...
def check_signals():
    ...
    for bar in ["30m", "15m"]:  # 先 30m 再 15m
        for symbol in symbols:
            ...
            if rev["type"] == "LOW":
                if not is_bull or df['low'].iloc[rev_idx] <= get_prev_swing_low(df, rev_idx):
                    continue

            if rev["type"] == "HIGH":
                if not is_bear or df['high'].iloc[rev_idx] >= get_prev_swing_high(df, rev_idx):
                    continue
            ...
            # === 15m 必須有 30m 共振 ===
            if bar == "15m":
                ref = recent_30m_reversals.get(symbol)
                if not ref or ref["type"] != rev["type"]:
                    continue
...
load_state()
update_today_top3()
send_telegram_message("🚀 OKX EMA 吞沒 + 順勢反轉（A+B 共振版）已啟動")
check_signals()
