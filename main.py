...
def process_signal(sym, df, timeframe):
    global sent_signals, gainers, losers
    ...
    bull_key = f"{sym}-{cur['ts']}-{timeframe}-bull"
    bear_key = f"{sym}-{cur['ts']}-{timeframe}-bear"
...
def check_signals():
    ...
    try:
        ...
        json.dump(sent_signals, open(STATE_FILE, "w"))
...
scheduler.add_job(check_signals, "cron", minute="2,17,32,47")  # 15m 與 30m 對齊
...
if __name__ == "__main__":
    if os.path.exists(STATE_FILE):
        try:
            sent_signals = json.load(open(STATE_FILE))
        except:
            sent_signals = {}

    scheduler.start()
    send_telegram("🚀 Bitunix EMA 監控（15m+30m）已啟動")
