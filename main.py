...
def update_today_top3():
    ...
    if today_date != now_date:
        ...
        try:
            ...
            df_vol['change_percent'] = pd.to_numeric(df_vol['change_percent'], errors='coerce')
            ...
            today_top3 = list(df_vol_up['symbol'].str.replace("_USDT","")) + list(df_vol_down['symbol'].str.replace("_USDT",""))
...
def check_signals(force=False):
    ...
    for bar in ["15m", "30m"]:
        for symbol in watch_symbols:
            ...
            prev_open, prev_close = df['open'].iloc[-2], df['close'].iloc[-2] if len(df) >= 2 else df['open'].iloc[-1], df['close'].iloc[-1]
